...
The `Console Log Formatter` adds `req-id` fact  (input http request id) and `trace-id` fact (current trace id) from `HostingLogScope`  or `ActionLogScope` independent of `IncludeScopes ` options.
